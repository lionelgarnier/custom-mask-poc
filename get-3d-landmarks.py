...
def set_front_view(plotter):
    ...
    # Position camera in front of the face model
    ...
    # Set camera style to 'trackball' for orbiting around the model
    plotter.enable_trackball_style()
    
    return plotter

def visualize_mesh_with_landmarks(mesh, landmarks, landmark_indices=None, plotter=None):
...
def create_3d_printable_extrusion(line_points, radius=0.1, resolution=30, output_path="extruded_line.stl"):
    ...
    Parameters:
    -----------
    line_points : array-like
        3D coordinates of points forming the line
    radius : float
        Radius of the tube extrusion
    resolution : int
        Resolution of the circular cross-section
    output_path : str
...
def process_face(mesh_path, show_landmarks_2d=False, show_landmarks_3d=False, show_mesh_and_landmarks=False,
                create_3d_print=False, extrusion_radius=0.1):
    ...
    cv2.imwrite("projection.png", image_bgr)  # Optional save
    ...
    # Get valid indices for landmarks
    valid_indices = np.where(valid_mask)[0]

    # Generate projection data before visualization
    pv_mesh = create_pyvista_mesh(mesh)
    ...
    if show_mesh_and_landmarks:
        if pv_plotter is not None:
            ...
            visualize_mesh_with_landmarks(mesh, valid_points_3d, valid_indices, pv_plotter)
            current_subplot += 1
        else:
            ...
            visualize_mesh_with_landmarks(mesh, valid_points_3d, valid_indices, landmarks_plotter)
    ...
    if create_3d_print:
        ...
        # Optionally visualize the extrusion
...
if __name__ == "__main__":
    ...
    # mesh_path = "D:/OneDrive/Desktop/masque/face_ron.obj"
